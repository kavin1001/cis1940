...
Addition (and many other operations) are only between values of the same numeric
type, and Haskell does not do implicit conversion. We can fix this with
functions like `fromIntegral` and `round`.

```Haskell
goodplus :: Double
goodplus = (fromIntegral j) + e

goodplus' :: Int
goodplus' = j + (round e)
```

Similarly, the following also gives an error:

```Haskell
baddiv = j / j
```

The `/` performs floating-point division only.
For integer division we use `div`.
...
<!-- Notice that `if`-_expressions_ are very different than `if`-_statements_. For
...
<!-- Another example, where we use our previous `hailstone` function:
...
hailstone sequence for `hailstone n`, the number obtained by
applying the hailstone function once to `n`. -->
